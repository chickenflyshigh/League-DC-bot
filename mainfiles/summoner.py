...
class Summoner:
    ...
    def initiate_v4(self): #does not give the id because we do not want to call for the summoner_info again using the API
        matches = self.match_ids_accid()
        if len(matches) == 0:
            return f'{self.name} has no matches to be found'
        if len(matches) != 0:
            self.prev_game = matches['matches'][0]['gameId']
        first = True
        for game in matches['matches']:
            match_1 = get.match_v4(game['gameId'])
            ## Finding whether or not the game was played today. We want to convert the unix time to AEST time
            if 'gameId' in match_1.keys(): #we check for this to ensure there is no error
                time_0 = float(match_1['gameCreation'])/1000
                time_0 = get.unix_to_utc(time_0)
                time_0 = get.utc_to_aest(time_0)
                game_playdate = (time_0 - datetime.timedelta(hours=reset_time)).date()
                # curr_time = utc_to_aest(datetime.datetime.utcnow())
                if game_playdate == (get.now_time()-datetime.timedelta(hours=reset_time)).date():
                    self.gameid = [(match_1['gameId'], game_playdate)] + self.gameid
                    players_in_game = match_1['participants']
                    for player in match_1['participantIdentities']:
                        if player['player']['summonerName'] == self.name:
                            participantId = player['participantId']
                            break
                    for player in players_in_game:
                        if player['participantId'] == participantId:
                            stats = player['stats']
                            tot_cs = float(stats['totalMinionsKilled'])+float(stats['neutralMinionsKilled'])
                            ...
                            self.duration.append(float(match_1['gameDuration'])/60)
                            if first:
                                self.prev_duration = float(match_1['gameDuration'])/60
                                self.prev_cs = tot_cs
                                first = False
                            if stats['win']:
                                self.wins += 1
        ...
        for game in matches:
            ...
            if 'metadata' in match_1.keys(): #we check for this to ensure there is no error
                ...
                if game_playdate == (get.now_time()-datetime.timedelta(hours=reset_time)).date():
                    ...
                    for player in players_in_game:
                        if player['player']['summonerId'] == self.summonerid:
    ...
    def update_v4(self):
        matches = self.match_ids_accid()
        if 'matches' not in matches.keys():
            return matches
        first_game = matches['matches'][0]['gameId']
        current_date = (get.now_time() - datetime.timedelta(hours=reset_time)).date()
        if len(matches) == 0: # only fails the function if this program has been running
            return f'{self.name} currently has no matches available.'
        def update(games):
            for game in games['matches']:
                if game['gameId'] == self.prev_game:
                    return
                match_1 = get.match_v4(game['gameId'])
                if 'gameId' in match_1.keys():
                    time_0 = float(match_1['gameCreation'])/1000
                    time_0 = get.unix_to_utc(time_0)
                    time_0 = get.utc_to_aest(time_0)
                    game_day = (time_0 - datetime.timedelta(hours=reset_time)).date() #minus 3 hrs 
        ...
        if len(self.gameid) != 0:
            if current_date == self.gameid[0][1]:
                if first_game == self.prev_game:
                    return f"{self.name} no change with same games in the same day"
                else:
                    update(matches)
                    self.prev_game = first_game
                    return f"{self.name} update with games played same day"
            else:
                self.pentaKills = 0
                self.doubleKills = 0
                self.tripleKills = 0
                ...
                self.losses = 0
                if first_game == self.prev_game:
                    return f"{self.name} resetted and all are zero"
                else:
                    update(matches)
                    self.prev_game = first_game
                    return f'{self.name} update with games played overnight'
        else:
            if self.prev_game == first_game:
                return f"{self.name} no games played today with previous game still the same. "
            else:
                update(matches)
                self.prev_game = first_game
                return f"{self.name} first game played today with update."
